...
// dilution), because the lexical (FTS) signal ranks it highly. The setup
...
describe("close-reading adjacency retrieval (fusion level)", () => {
  it("surfaces the exact-answer chunk even when vector under-ranks it", () => {
    ...
    // The answer climbs from 4th to the top two, beside its setup chunk, and
    // ahead of the vector-only near misses.
    expect(fused.slice(0, 2)).toEqual(
      expect.arrayContaining(["chunkN", "chunkN-1"]),
    );
  });
});
