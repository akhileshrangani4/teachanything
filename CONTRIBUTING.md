...
The seed creates an admin, a professor with 3 chatbots and 7 sample files (PDF, DOCX, TXT, MD, JSON, CSV, PPTX — slides are chunked on slide boundaries and speaker notes are extracted), and a pending user for testing the approval workflow. Credentials are printed to the console.

Chatbots can also ingest entire websites via the built-in web crawler — add a root URL, set depth and page limits, and it auto-discovers + indexes pages into the same RAG knowledge base. Works offline (no extra env vars required).
